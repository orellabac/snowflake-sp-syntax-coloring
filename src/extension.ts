...
function findOccurances(doc: vscode.TextDocument, line: number, char: string): Array<number> {
  ...
  var matches = (content.text + "hack").split(char).reduce((acc:any, p) => {
    var len = p.length + 1;
    if (acc.length > 0) {
...
function findPrevious(doc: vscode.TextDocument, line: number, char: string, start_index?: number, nest_char: string | undefined = undefined, nested: number = 0): vscode.Position | undefined {
  ...
  var occurances = findOccurances(doc, line, char).filter(n => n <= <any>start_index);
  var nests = nest_char ? findOccurances(doc, line, nest_char).filter(n => n <= <any>start_index) : [];
...
function findSingleSelect(s: vscode.Selection, doc: vscode.TextDocument, char: string, outer?: boolean, multiline?: boolean) {
  ...
  let next:any = matches.find(a => a > character);
  ...
  if (matches.length > 1 && matches.length % 2 === 0) {
    ...
    //Automatically grow to outer selection
    if (!outer &&
      start_pos.isEqual(s.anchor) && end_pos && 
...
function switchQuotes() {
  ...
  if (editor)
  {
    editor.selections = <any>original_sel;
  }
...
function matchingSelect({start_char, end_char, outer = false}: MatchingSelectOptions) {
  ...
  editor.selections = sel.map(s => {
    ...
    let start:any = starts.find(a => a > character);
    let end:any = ends.find(a => a > character);
